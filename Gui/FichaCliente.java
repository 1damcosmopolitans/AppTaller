...
public class FichaCliente extends JFrame {
	...
	private void initialize() {
		...
		/**
		 * Para impedir que se pueda introducir numeros Y un límite de
		 * carácteres de 15
		 */
		txtFldNombre.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e1) {
				...
				if ((c1 < 'a' || c1 > 'z') && (c1 < 'A' || c1 > 'Z'))
					e1.consume();
				if (txtFldNombre.getText().length() == limit1)
					e1.consume();
				if (txtFldNombre.getText().matches(".*\\d+.*")) { // Validacion
																	// de los
																	// simbolos
																	// raros
		...
		txtFldApellido1.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e2) {
				...
				if ((c2 < 'a' || c2 > 'z') && (c2 < 'A' || c2 > 'Z'))
					e2.consume();
				if (txtFldApellido1.getText().length() == limit2)
					e2.consume();
				if (txtFldApellido1.getText().matches(".*\\d+.*")) { // Validacion
																		// de
																		// los
																		// simbolos
																		// raros
		...
		txtFldApellido2.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e3) {
				...
				if ((c3 < 'a' || c3 > 'z') && (c3 < 'A' || c3 > 'Z'))
					e3.consume();
				if (txtFldApellido2.getText().length() == limit3)
					e3.consume();
				if (txtFldApellido2.getText().matches(".*\\d+.*")) { // Validacion
																		// de
																		// los
																		// simbolos
																		// raros
		...
		txtFldDireccion.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e5) {
				int limit4 = 50;
				if (txtFldApellido2.getText().length() == limit4)
					e5.consume();
			}
		});
		...
		/**
		 * Para impedir que se pueda introducir letras Y un límite de carácteres
		 * de 9
		 */
		txtFldTelefono.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e6) {
				...
				if (c4 < '0' || c4 > '9')
					e6.consume();
				if (txtFldTelefono.getText().length() == limit5)
					e6.consume();
			}
		});
		...
		txtFldEmail.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e7) {
				int limit6 = 25;
				if (txtFldEmail.getText().length() == limit6)
					e7.consume();
		...
		/**
		 * AL RELLENAR EL FORMULARIO VALIDADO GUARDAMOS LOS DATOS***3 CMPOS
		 * OBLIGATORIOS
		 */
		...
		btnGuardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				if (txtFldDni.getText().equals("") || txtFldDni.getText() == null || txtFldNombre.getText().equals("")
						|| txtFldNombre.getText() == null || txtFldApellido1.getText().equals("") || txtFldApellido1 == null
						|| txtFldDireccion.getText().equals("") || txtFldDireccion.getText() == null) {
					JOptionPane.showMessageDialog(null,"ERROR AL AÑADIR CLIENTE - debes rellenar los campos obligatorios");
					...
					if (!validarEmail(txtFldEmail.getText())) {
						...
						try {
							...
							if (modo) {
								...
								ControlClientes.Aniadir(txtFldDni.getText(), txtFldNombre.getText(),
										txtFldApellido1.getText(), txtFldApellido2.getText(), txtFldDireccion.getText(),
										txtFldEmail.getText(), txtFldTelefono.getText(), txtAreaComentarios.getText());
								JOptionPane.showMessageDialog(null, "Se añadio con exito el cliente", "AÑADIR CLIENTE",
										JOptionPane.INFORMATION_MESSAGE);
							} else {
								ControlClientes.Editar(dniCliente, txtFldDni.getText(), txtFldNombre.getText(),
										txtFldApellido1.getText(), txtFldApellido2.getText(), txtFldDireccion.getText(),
										txtFldEmail.getText(), txtFldTelefono.getText(), txtAreaComentarios.getText());
								JOptionPane.showMessageDialog(null, "Se modificó con exito el cliente",
										"AÑADIR CLIENTE", JOptionPane.INFORMATION_MESSAGE);
							}
						} catch (Exception e) {
							JOptionPane.showMessageDialog(null, e.getMessage(), "ERROR AL AÑADIR CLIENTE",
		...
		btnGuardar.setBounds(256, 488, 185, 50);
